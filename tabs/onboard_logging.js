...
TABS.onboard_logging.initialize = function (callback) {
    ...
    function update_bar_width(bar, value, total) {
        update_bar_width(bar, value, total, "", false);
    }
    ...
    function update_legend(el, value, label, valuesAreKilobytes) {
        if(value > 0) {
            el.text((label ? label + ": " : "") + (valuesAreKilobytes ? formatFilesizeKilobytes(value) : formatFilesizeBytes(value)));
        } else {
            el.css({
                display: 'none'
            });
    ...
    function update_html() {
        ...
        update_bar_width($(".tab-onboard_logging .sdcard-other"), SDCARD.totalSizeKB - SDCARD.freeSizeKB, SDCARD.totalSizeKB);
        update_bar_width($(".tab-onboard_logging .sdcard-free"), SDCARD.freeSizeKB, SDCARD.totalSizeKB);

        update_legend(
            $(".sdcard-busy-legend"),
            SDCARD.totalSizeKB - SDCARD.freeSizeKB,
            chrome.i18n.getMessage('unavailableSpace'),
            true
        );
        update_legend(
            $(".sdcard-free-legend"),
            SDCARD.freeSizeKB, 
            chrome.i18n.getMessage('freeSpaceForLogs'),
            true
        );
